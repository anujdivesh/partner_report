...
def first_page():
    ...
    ax.text(0.01, 1.12,"Riskscape Infographic", transform=ax.transAxes,fontsize=16, verticalalignment='top', color='#BB3F3F')
    ax.text(0.01, 1.09,"Cook Islands Government", transform=ax.transAxes,fontsize=8, verticalalignment='top', color='#BB3F3F')
    ax.text(0.01, 1.07,"Contact: contactus@spc.int", transform=ax.transAxes,fontsize=6, verticalalignment='top', color='#BB3F3F')
    ax.axis('off')
    axline = fig.add_axes([0.001, 0.90, 0.99, 0.02])
    axline.axhline(y=0.5, color='#BB3F3F', linestyle='-',linewidth = 15)
    axline.axis('off')
    axline.text(0.11, 1.043,"Riskcape Report for Cook Islands, model run: 2023-05-30 16:00 UTC", fontweight='bold', transform=ax.transAxes,fontsize=8, verticalalignment='top', color='white',)
